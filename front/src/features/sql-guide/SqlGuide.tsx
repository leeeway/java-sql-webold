...
function SqlGuide(): React.JSX.Element {
  const [guidData, setGuidData] = useState<SqlGuidItem[]>([]);
  const [menuKey, setMenuKey] = useState<string>('stage');

  useEffect(() => {
    const client = createClient();

    void client.get('/sql/guid').then((response) => {
      setGuidData(response.jsonData.data as SqlGuidItem[]);
    });
  }, []);

  const copyText = (script: string): void => {
    copy(script);
