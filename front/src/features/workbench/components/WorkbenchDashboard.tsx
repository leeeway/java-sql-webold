...
import type { WorkbenchDashboardSection, WorkbenchDashboardItem } from '@/types/api';
...
function getStatusMeta(status: string): { color: string; text: string } {
  switch (status) {
    case 'ok':
...
function renderUpdatedAt(updatedAt?: string): string {
  if (!updatedAt) {
    return '等待首次加载';
  }
...
function WorkbenchDashboard({ data, error, loading, onRefresh }: WorkbenchDashboardProps): React.JSX.Element {
  if (loading && !data) {
    return (
