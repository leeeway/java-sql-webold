...
interface AdminDashboardProps {
  data: any;
  filter: { range: string; grain: string };
  loading: boolean;
  updatedAt: string;
  onRangeChange: (value: string) => void;
  onGrainChange: (value: string) => void;
  onRefresh: () => void;
  onViewDynamicPoolDetail?: (record: any) => void;
}
      ...
      <div className="admin-dashboard-grid">
          ...
          {userChartData.length > 0 ? (
            <>
              ...
              <Table
                ...
                pagination={false}
                rowKey={(record: any) => `rank-${record.rank}-${record.userName}`}
                size="small"
              />
            </>
          ) : (
        ...
        <section className="admin-dashboard-card admin-dashboard-recent">
          ...
          <Table
            ...
            pagination={false}
            rowKey={(record: any) => `recent-${record.queryTime || 'unknown'}-${record.queryName || 'unknown'}`}
            size="small"
          />
        </section>
      </div>
