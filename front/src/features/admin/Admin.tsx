...
interface AdminState {
  ...
  dashboardFilter: { range: string; grain: string };
  sessionDetailVisible: boolean;
  sessionDetailLoading: boolean;
  sessionDetailError: string;
  sessionDetailServer: any;
  sessionDetailRows: any[];
  queryLogCursor: any;
  connList: any[];
  serverRuntimeMap: Record<string, any>;
  serverTestResults: Record<string, any>;
  testingServerCode: number | null;
  resettingServerCode: number | null;
  syncingServerDatabases: boolean;
  latestServerDatabaseSyncTime: string;
  testingConfigServer: boolean;
  configVisible: boolean;
  userAddVisible: boolean;
  confirmLoading: boolean;
  userGroupAddVisible: boolean;
  permissionAddVisible: boolean;
  druidList: any[];
  userList: any[];
  ...
  linkVisible: boolean;
  issuedLinkTitle: string;
  issuedLinkData: any;
  userGroupList: any[];
  dbPermissionList: any[];
  inputData: Record<string, any>;
  permissionEditGroupCode: number;
  permissionEditServerList: any[];
  userGroupEditGroupCode: number;
  userGroupEditUserList: any[];
  token: string;
}
...
function Admin() {
  ...
  const loadQueryLog = async (options: { pageSize?: number; direction?: string; cursorCode?: number } = {}) => {
    const client = createClient();
    const query = new URLSearchParams({
  ...
  const loadServerManagement = async (overrides: { keyword?: string; serverType?: string } = {}) => {
    const nextKeyword =
  ...
  const loadTargetSessionDetails = async (serverRecord: any, options: { keepRows?: boolean } = {}) => {
  ...
  const userListColumns = [
    ...
    },
    {
      ...
      render: (value, record) => {
        if (!value) {
          return <Tag className="admin-soft-tag">无</Tag>;
        }
    ...
    },
    {
      ...
      render: (value) => {
        ...
        return <Tag className="admin-soft-tag">未申请</Tag>;
      },
    },
    {
      title: '令牌到期时间',
  ...
  const menuItems = [
    ...
    { key: '8', icon: <SafetyCertificateOutlined />, label: 'OIDC / SSF' },
  ...
  const menuMeta = {
    ...
    '8': {
      kicker: 'Identity',
      title: 'OIDC / SSF',
      subtitle: '管理 OpenID Connect 连接与 Shared Signals 事件流',
    },
  };
        ...
        <Content className="admin-content">
          ...
          {state.menuSelect === '4'
            ? renderPageSection(
                ...
                <Table
                  ...
                  pagination={false}
                  rowKey={(record: any) => `query-log-${record.code}`}
                  size="small"
                />,
              )
            : null}
          ...
          {state.menuSelect === '8'
            ? renderPageSection(
                'OIDC / SSF',
                null,
                <OidcSsfPanel token={state.token} />,
              )
            : null}
