...
function shouldRedirectToLogin(response: ApiResponseLike): boolean {
  if (!response) {
    return false;
  }
...
export function createClient(): FetchHttpClient {
  const client = new FetchHttpClient(config.serverDomain);
  client.addMiddleware(json());
  client.addMiddleware(() => (response: ApiResponseLike) => {
